...
points_to_be_evaluated <- 
  expand.grid(x = seq(-50, 50, by = 10), y = seq(-50, 50, by = 10)) 
...
f(x = points_to_be_evaluated$x, y = points_to_be_evaluated$y)

# ggplot(data = points_to_be_evaluated, aes(x = x, y = y, color = id)) +
  ggplot(data = points_to_be_evaluated, aes(x = x, y = y)) +
  stat_vector_field(fun = f, scaling_factor = 5, arrow = arrow(length = unit(1, "mm"))) +
...
When creating new stats it is usually a good idea to provide an accompanying `geom_*()` constructor as well as the `stat_*()` constructor, because most users are accustomed to adding plot layers with geoms rather than stats. We'll show what a geom_chull() function might look like in Section 20.3.
