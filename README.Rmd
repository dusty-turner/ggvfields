...
`r badger::badge_repostatus("Active")`  `r badger::badge_code_size()`  `r badger::badge_codecov()`  

<!-- badger::badge_cran_checks(pkg = "ggvfields") -->
<!-- [![CRAN status](https://www.r-pkg.org/badges/version/ggvfields)](https://CRAN.R-project.org/package=ggvfields) -->
...
As with other functions, we can increase the granularity of the visualization with the `n` parameter.
...
**ggvfields** offers two techniques for smoothing noisy vector field data `geom_stream_smooth()` and `geom_vector_smooth()`
...
- `formula`: A formula specifying the linear model used to approximate the scalar field. Defaults to `z ~ x + y + I(x^2) + I(y^2)`.
- `type`: Type of visualization, either `"vector"` (default, arrows) or `"stream"` (streamlines).
- `n`: The resolution of the grid to evaluate the gradient field.
- `center`, `normalize`, and other graphical options for controlling visual appearance similar to the other gradient and vector functions above. 
