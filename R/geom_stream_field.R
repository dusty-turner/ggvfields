...
#'   axis). Defaults to 11, resulting in an \eqn{n \times n} grid of seed points.
...
#' @param center Logical. If `TRUE`, centers the seed points or the resulting streamlines
#'   so that the original (x, y) becomes the midpoint. Default is `TRUE`.
#' @param normalize Logical. If `TRUE`, stream lengths are normalized based on grid spacing.
#'   If `FALSE`, a default arc length is used. Default is `TRUE`.
...
#'   starting point) of each streamline. This can help to indicate the origin of the
#'   computed path. Default is `FALSE`.
#' @param eval_point Logical. If `TRUE`, a point is drawn at the evaluation point,
#'   which corresponds to the original untransformed seed point prior to any centering
#'   or normalization. This allows for comparison between the original and the
#'   transformed positions. Default is `FALSE`.
#' @param geom The geometric object used to draw the streamline. Defaults to [GeomStream].
#' @param ... Other arguments passed to [ggplot2::layer()] and the underlying geometry/stat.
#'
#' @return A ggplot2 layer that computes the streamlines over the specified domain
#'   and draws them, optionally with arrowheads and with points at the tail and/or
#'   evaluation positions.
#'
#' @details
#' The streamlines are generated by numerically integrating the vector field defined by
#' `fun(x, y)`. The computed streamlines are then passed to [GeomStream] for rendering.
#' When `tail_point = TRUE`, a point is drawn at the start of each streamline. When
#' `eval_point = TRUE`, a point is drawn at the original (untransformed) seed point,
#' allowing you to see the location before centering or normalization is applied.
#'
#' @section See Also:
...
#' ggplot() +
#'   geom_stream_field(fun = rotational_field, xlim = c(-2, 2), ylim = c(-2, 2),
#'                     tail_point = TRUE)
#'
#' # Create a stream field layer with evaluation points
#' ggplot() +
#'   geom_stream_field(fun = rotational_field, xlim = c(-2, 2), ylim = c(-2, 2),
#'                     eval_point = TRUE)
#'
