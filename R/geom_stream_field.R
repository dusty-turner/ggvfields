...
#'
#' `geom_streamfield()` creates a geom layer that visualizes stream-like data with wiggly paths. This is useful for representing flows, trajectories, or any data that benefits from a dynamic, flow-based visualization.
#'
...
#'
#' @details
#' The `geom_streamfield` function integrates seamlessly with `ggplot2`, allowing users to add dynamic and visually appealing streamlines to their plots. It leverages the `StatStreamField` for computing the stream data and `GeomWigglyStream` for rendering the paths with optional arrows indicating flow direction.
#'
...
#'
#' @seealso
#' - `\code{\link{StatStreamField}}`: Statistical transformation used by `geom_streamfield`.
#' - `\code{\link{GeomWigglyStream}}`: Geometric object responsible for rendering the streamlines.
#' - `\code{\link[ggplot2]{geom_path}}`: Base geom that `geom_streamfield` builds upon.
#'
#' @examples
#' \dontrun{
#' library(ggplot2)
#' }
#'
#' @rdname geom_streamfield
#' @export
geom_streamfield <- function(mapping = NULL, data = NULL,
                              stat = "streamfield", position = "identity",
                              ...,
                              na.rm = FALSE,
                              show.legend = TRUE,
                              inherit.aes = TRUE,
...
stat_streamfield <- function(mapping = NULL, data = NULL,
                              stat = "streamfield", position = "identity",
                              ...,
                              na.rm = FALSE,
                              show.legend = TRUE,
...
#' @usage NULL
#' @export
# Define the StatStreamField object and computation for stream plots
StatStreamField <- ggproto("StatStreamField", Stat,

                            default_aes = aes(group = after_stat(pt), color = after_stat(max(l)/max_t)),
