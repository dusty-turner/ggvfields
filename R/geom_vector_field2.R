#' Geom Vector Field 2
#'
#' @title Geom Vector Field 2
#'
#' @name geom_vector_field2
#'
#' @description
...
#'   length 2 (representing \eqn{(x, y)} coordinates) and return a numeric vector of length 2
#'   (\eqn{(dx, dy)}) indicating the direction of the vector at that point. **(Required)**
...
#'   Useful for symmetric flows. **Defaults to `FALSE`.**
#' @param normalize Logical. If `TRUE`, stream lengths are normalized based on grid spacing.
#'   If `FALSE`, a default arc length is used. Defaults to `TRUE`.
