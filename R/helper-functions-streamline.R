...
solve_streamline <- function(initial_state, fun, times, xlim, ylim) {
  parameters <- list(fun = fun, xlim = xlim, ylim = ylim)

  result <- ode(y = initial_state, times = times, func = stream_ode, parms = parameters, method = "rk4")
...
stream_ode <- function(time, state, parameters) {
  x <- state[1]
  y <- state[2]
