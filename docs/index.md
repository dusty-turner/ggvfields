# ggvfields ![](reference/figures/ggvfields_hex.png)
...
- **[`geom_vector_field()`](/reference/geom_vector_field.md)**: Computes
  vector fields from a user-defined function over the domain
...
- **[`geom_vector_field2()`](/reference/geom_vector_field.md)**: Similar
  to [`geom_vector_field()`](/reference/geom_vector_field.md), but maps
...
feature of [`geom_vector2()`](/reference/geom_vector.md) enhances
interpretability by using actual vector lengths to represent magnitude.
The legend reflects the scaling and ensures correct interpretation.

- **[`geom_vector_field()`](/reference/geom_vector_field.md) options**
...
- **[`geom_stream_field()`](/reference/geom_stream_field.md)**: Computes
  stream fields from a user-defined function and maps the average speed
  to color. Average speed is the overall rate at which a particle
  traverses the shown stream. If the displacement vector has length
  $|\mathbf{d}|$ and it takes time $t$, the integration time of streams,
  to traverse that distance, then the average speed is given by
...
- **[`geom_stream_field2()`](/reference/geom_stream_field.md)**: Similar
  to [`geom_stream_field()`](/reference/geom_stream_field.md), but
...
- **[`geom_stream_field()`](/reference/geom_stream_field.md) options**

[`geom_stream_field()`](/reference/geom_stream_field.md) maintains
similar options to
[`geom_vector_field()`](/reference/geom_vector_field.md). Some arguments
yield slightly different behavior.
...
The [`geom_gradient_field()`](/reference/geom_gradient_field.md)
function computes and visualizes gradient fields derived from scalar
functions and displays the gradient vector field of a scalar function,
$f(x,y)$. The gradient is given by:
...
- **Gradient Fields as Vectors** The resulting vector field has all the
  same defaults and same options as
  [`geom_vector_field()`](/reference/geom_vector_field.md)
...
The [`geom_potential()`](/reference/geom_potential.md) function computes
and visualizes the scalar potential function for a given conservative
vector field. The input function must represent a 2D vector field and
the output is the corresponding potential function. If the input field
is not conservative, the function checks this condition numerically
based on a tolerance parameter. The tolerance determines how strictly
the field must satisfy the conservation condition.
...
- **[`geom_vector()`](/reference/geom_vector.md)**: By default, this
  maps the norm (magnitude) of a vector to its color.
...
[`geom_vector()`](/reference/geom_vector.md) also supports both
`xend`/`yend` format as well as `fx`/`fy` format.
...
- **[`geom_vector2()`](/reference/geom_vector.md)**: Maps the norm of a
  vector directly to its length. This provides a more intuitive
  representation of magnitude. This is done by mapping
  `length = after_stat(norm)` by default.
...
Both [`geom_vector()`](/reference/geom_vector.md) and
[`geom_vector2()`](/reference/geom_vector.md) also support polar
...
[`geom_stream_smooth()`](/reference/geom_stream_smooth.md) and
[`geom_vector_smooth()`](/reference/geom_vector_smooth.md)

[`geom_stream_smooth()`](/reference/geom_stream_smooth.md) uses a
dynamical systems approach and
[`geom_vector_smooth()`](/reference/geom_vector_smooth.md) offers a
multivariate regression approach that accounts for uncertainty.
...
\widehat{dx} \\
\widehat{dy}
\end{pmatrix} = \beta_{0} + \beta_{1}x + \beta_{2}y + \beta_{3}xy$$
...
[`geom_gradient_smooth()`](/reference/geom_gradient_smooth.md) creates a
**smoothed gradient field** from raw scalar data using a fitted linear
model. This function estimates gradients when only scalar values (`z`)
are observed at spatial locations (`x`, `y`). It is designed for cases
where you have scalar data and wish to estimate the gradient.
...
To illustrate how
[`geom_gradient_smooth()`](/reference/geom_gradient_smooth.md) can adapt
to nonlinear surfaces, we can change the formula used to fit the scalar
field and switch to a streamline visualization using `type = "stream"`.
The example below uses a smooth but noisy scalar function that generates
...
![](reference/figures/README-unnamed-chunk-33-1.png)
...
![](reference/figures/README-unnamed-chunk-34-1.png)
